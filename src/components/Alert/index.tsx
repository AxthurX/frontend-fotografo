...
const AlertComponent = ({
	...
	return (
		<div
			className={`${
				color === 'success'
					? 'border-royalblue-700 bg-royalblue-800 text-royalblue-800'
					: color === 'error'
						? 'border-2 border-red-500 border-opacity-50 bg-red-200'
						: color === 'warning'
							? 'border-beige-400'
							: ''
		...
		>
			<div className='flex items-center '>
				...
				<div className='ml-3 font-bold text-md'>{title}</div>
			</div>
			<div className='mt-2 ml-9 text-sm'>{label}</div>
			<div className='mt-2 ml-9 text-sm'>{children}</div>
						...
						color === 'success'
							...
							: color === 'error'
								...
								: color === 'warning'
									...
									: ''
					} absolute top-2 right-2 rounded-lg p-1.5 transition-colors duration-300 ease-in-out`}
