...
>(({ className, variant = "outline", size = "icon", ...props }, ref) => {
  ...
  return (
    <Button
      ...
      size={size}
      className={cn(
        "absolute size-12 rounded-full bg-beige-50 opacity-50 max-sm:hidden max-sm:size-10",
        orientation === "horizontal"
...
>(({ className, variant = "outline", size = "icon", index, ...props }, ref) => {
  ...
  const handleSelectedIndexClassName = () => {
  ...
  return (
    <Button
      ...
      size={size}
      className={cn(
        "mx-1 size-4 rounded-full bg-beige-50 " +
          handleSelectedIndexClassName(),
        className,
      )}
      onClick={() => scrollTo(index)}
      {...props}
    >
      <span className="sr-only">Mover para slide {index + 1}</span>
    </Button>
  );
});
