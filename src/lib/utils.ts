...
export function onErrorLogin(e: unknown) {
	const error: { message: string } = { message: '' };
	console.error(e);
	...
	if (e instanceof AxiosError)
		if (e.response)
			if (e.response.data)
				error.message = e.response.data.message || e.response.data.err;
			else error.message = 'Erro interno';
...
export function isAuthorized(pathname: string): boolean {
	const unauthorized_routes = ['/entrar', '/cadastrar'];
	const is_album_route = pathname.startsWith('/albums/');
	return !unauthorized_routes.includes(pathname) && !is_album_route;
}
