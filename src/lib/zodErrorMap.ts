	...
	switch (error.code) {
		case z.ZodIssueCode.invalid_type: {
			if (error.received == 'undefined' || error.received == 'null')
				...
				return {
					message: `Este campo deveria ser ${error.expected}, mas é ${error.received}.`,
				};
			break;
		}
				...
				const minType =
					error.type === 'string'
						...
						: error.minimum === 1
							...
							: 'dígitos';
				return {
					message: `Este campo deve ter no mínimo ${error.minimum} ${minType}.`,
				};
			...
			const maxType =
				error.type === 'string'
					...
					: error.maximum === 1
						...
						: 'dígitos';
			return {
				message: `Este campo deve ter no máximo ${error.maximum} ${maxType}.`,
			};
		}
